...
export const updateRBACFromRole = (role) => {
  ...
  if (role.routes && role.routes.includes("*")) {
...
export const hasRouteAccess = (userRole, path) => {
  ...
  // Check if the role has wildcard access
  if (ROLE_PERMISSIONS[userRole].routes && ROLE_PERMISSIONS[userRole].routes.includes("*")) {
    return true;
  }
  ...
  // Check if the role has access to this specific route
  if (ROLE_PERMISSIONS[userRole].routes && ROLE_PERMISSIONS[userRole].routes.includes(path)) {
    return true;
  }
...
if (typeof window === "undefined") {
  ...
  initializeRBAC().catch(console.error);
}