...
export default function user() {
  ...
  const fetchFields = async () => {
    ...
    localStorage.setItem(`user_fields`, JSON.stringify(defaultFields.map(f => f.name)));
  };

  const fetchData = async () => {
    const searchQuery = searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : '';
    const res = await fetch(`/api/user?page=${page}&limit=${limit}${searchQuery}`);
  ...
  const toggleFieldSelection = (field) => {
    ...
    localStorage.setItem(`user_fields`, JSON.stringify(newFields.map(f => f.name)));
  };
                      ...
                      <div className="edit_de_td">
                       <button 
                        ...
                        >
                          ...
                          <Link href={`/manager/user/edit/${item._id}`}>
